...
> 基于 [roseforyou/cf-outlook-email](https://github.com/roseforyou/cf-outlook-email) Fork 定制，以下为新增/修改内容。

### 🚀 自动部署
- GitHub Actions 自动部署：Push 到 `main` 分支即自动部署到 Cloudflare Workers，无需本地环境

### 📬 仪表盘邮件速览（3 列式）
- **左列** — 邮箱列表（带头像、分组标签、搜索框）
- **中列** — 选中邮箱后显示收件箱邮件列表
- **右列** — 点击邮件显示完整详情（支持 HTML 渲染）
- 📋 一键复制邮箱地址 / 🔄 刷新邮件列表
- 按分组下拉筛选 + 按邮箱名实时搜索
- 分组标签可直接点击切换分组，无需跳转页面

### 🔑 一键复制验证码
- 自动识别邮件中的验证码（4-8 位，支持中英文关键词匹配 + 兜底策略）
- 邮件详情标题下方显示 `🔑 检测到验证码: XXXXXX [复制验证码]`
- 仪表盘邮件速览和邮件查看页面均生效

### 📦 导入导出自定义格式
- **导出**：5 种预设格式 + 自定义模板（`{email}` `{password}` `{client_id}` `{refresh_token}` 自由组合），切换格式实时预览
- **导入**：3 种预设格式 + 自定义字段顺序（4 个下拉框自由排列）
- 默认格式：`邮箱----密码----refresh_token----client_id`
